...
public class SenbonzakuraSkills {
    ...
    public static void applyPetalBleed(LivingEntity target, Player attacker, MythicBladesPlugin plugin) {
        ...
        new BukkitRunnable() {
            ...
            @Override public void run() {
                if (!target.isValid() || target.isDead() || !target.isValid() || t >= ticks) { cancel(); return; }
                target.damage(bleedDmg, attacker);
                // One orbit particle instead of 4
                ...
                ParticleUtils.spawn(l.getWorld(), Particle.CHERRY_LEAVES,
                    l.clone().add(Math.cos(angle) * 0.7, 0, Math.sin(angle) * 0.7), 1, 0.02, 0.08, 0.02, 0);
                t++;
            }
    ...
    public static void scatter(Player player, MythicBladesPlugin plugin) {
        ...
        new BukkitRunnable() {
            ...
            @Override public void run() {
                ...
                // Every 3 ticks (was every tick)
                if (t % 3 == 0) {
                    world.spawnParticle(Particle.CHERRY_LEAVES, center, 4, 3, 1, 3, 0.15);
                    world.spawnParticle(Particle.ENCHANT, center, 2, 2, 1, 2, 0.3);
                }
                t++;
            }
    ...
    private static void detonateScatter(Player player, Location center, World world,
        ...
        world.spawnParticle(Particle.CHERRY_LEAVES, center, 40, 5, 2, 5, 0.3);
        world.spawnParticle(Particle.ENCHANT,       center, 15, 4, 2, 4, 0.8);
        world.spawnParticle(Particle.END_ROD,       center, 8,  3, 1, 3, 0.15);
    ...
    public static void petalPrison(Player player, MythicBladesPlugin plugin) {
        ...
        // 4 columns instead of 8 — still looks like a cage
        for (int i = 0; i < 4; i++) {
            double a = Math.toRadians(90 * i);
            Location col = center.clone().add(Math.cos(a) * radius, 0, Math.sin(a) * radius);
            for (int y = 0; y < 8; y++) {
                final Location pt = col.clone().add(0, y * 0.5, 0);
                final long delay = y * 1L;
                new BukkitRunnable() {
                    @Override public void run() {
                        world.spawnParticle(Particle.CHERRY_LEAVES, pt, 1, 0.1, 0.05, 0.1, 0.01);
                    }
    ...
    public static void kageyoshi(Player player, MythicBladesPlugin plugin) {
        ...
        world.spawnParticle(Particle.CHERRY_LEAVES, center, 25, 3, 2, 3, 0.3);
        ...
        new BukkitRunnable() {
            ...
            @Override public void run() {
                ...
                // Orbit blades — only every 3 ticks
                if (tick % 3 == 0) {
